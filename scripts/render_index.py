...
def build_index_md(model, now, commit):
    ...
    # -- A ----------------------------------------------------------------
    W("## A. Achievementy (źródło dowodów)")
    W("")
    W("| Kod | Rola | Okres | Waga | Skills | Stories |")
    W("| --- | --- | --- | --- | --- | --- |")
    for a in prof:
        role = f"`{model.role_of(a)}`"
        period = model.ach[a]["period"]
        importance = model.ach[a]["importance"]
        skills = ", ".join(f"`{s}`" for s in model.a2sk.get(a, []))
        stories = ", ".join(f"`{s}`" for s in model.a2st.get(a, []))
        W(f"| `{a}` | {role} | {period} | {importance} | {skills} | {stories} |")
    W("")
    W("---")
    W("")

    # -- B ----------------------------------------------------------------
    W("## B. Kompetencje (wagi i dowody)")
    W("")
    W("| Kod | Poziom | Waga | Dowodów | Słowa kluczowe |")
    W("| --- | --- | --- | --- | --- |")
    for s in sorted(model.skill.keys()):
        level = model.skill[s]["level"]
        importance = model.skill[s]["importance"]
        count = len(model.s2a.get(s, []))
        keywords = model.skill[s].get("keywords", [])
        kw_str = ", ".join(f"`{k}`" for k in keywords)
        W(f"| `{s}` | {level} | {importance} | {count} | {kw_str} |")
    W("")
    W("---")
    W("")

    # -- C ----------------------------------------------------------------
    W("## C. Gotowe bullety z STORY")
    W("")
    for st in sorted(model.story.keys()):
        W(f"### `{st}` · {model.story[st]['title']}")
        achs_for_st = [a for a, stories in model.a2st.items() if st in stories]
        ach_codes = ", ".join(f"`{a}`" for a in achs_for_st)
        W(f"Powiązane: {ach_codes}")
        W(f"Długość: {len(model.story[st]['bullet'])} znaków")
        W("")
        W("```")
        W(model.story[st]["bullet"])
        W("```")
        W("")
    ...
    # -- D ----------------------------------------------------------------
    W("## D. Impact dla achievementów bez STORY")
    W("")
    W("Ten materiał jest dopuszczalnym materiałem — przenoś liczby dosłownie.")
    W("")
...
def main():
    ...
    idx_path = os.path.join(ROOT, INDEX_FILENAME)
    old = ""
    if os.path.isfile(idx_path):
        with open(idx_path, encoding="utf-8") as fh:
            old = fh.read()

    # Porownanie - ignoruj stopke (czasnik i commit)
    def body(t):
        return "\n".join(
            l for l in t.split("\n")
    ...
    if body(old) != body(content):
        print(f"  zaktualizowano: {INDEX_FILENAME}")
        with open(idx_path, "w", encoding="utf-8") as fh:
            fh.write(content)
    else:
        print(f"  bez zmian: {INDEX_FILENAME}")
