...
def build_wiring_md(model, now, commit):
    ...
    a("")
    a("**Ten plik nie jest zrodlem do parsowania.** Dlugie listy sa skracane")
    a("do kilku pozycji z licznikiem reszty (`(+N)`) — dotyczy to m.in. pola")
    a("`obecne:` przy kompetencjach, `slowa kluczowe`, `zakres` i Sygnalow.")
    a("Zadna z tych list nie jest zamknieta. Program, ktory potraktuje je jako")
    a("kompletne, po cichu odrzuci poprawne pary ACH<->SKILL — bez bledu")
    a("i bez sladu w logu.")
    a("")
    a("Maszynowym wejsciem jest `dist/vault.json`: ten sam model, wszystkie")
    a("listy w calosci, stabilny `schema_version` i stempel commita.")
    a("")
